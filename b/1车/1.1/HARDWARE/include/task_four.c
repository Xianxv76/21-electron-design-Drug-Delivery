	...
	else
	  {
		...
		{  
   		   USART_Cmd(USART3, DISABLE);
  			 ...
  			 turn_flag = 0; 
			   if(( (( right_motor.raw_value + left_motor.raw_value )/2) - two.ech_hold[two.ech_i] ) < 1200)//two.road_act[two.ech_i] )  //第二个路口，屏蔽openmv
				 {
					 right_set = speed_set;
					 left_set = speed_set;
					 USART_Cmd(USART3, DISABLE);
...
//				}
				 if(( (( right_motor.raw_value + left_motor.raw_value )/2) - two.ech_hold[two.ech_i] ) < 1200)//stwo.road_act[two.ech_i] )  //第二个路口，屏蔽openmv
				 {
					 right_set = speed_set;
					 left_set = speed_set;
					 USART_Cmd(USART3, DISABLE);
